...
class Decoder(nn.Module):
    def __init__(self, z_dim, x_dim, y_dim, c_dim, hidden_dim):
        ...
        self.proj_network = torch.nn.Linear(self.z_dim + self.c_dim, self.hidden_dim)
        self.projout_network = torch.nn.Linear(self.hidden_dim, self.y_dim)

    def decode(self, c, z, x, y_pred_len, train=False, y=None):
        y_out = []
        zc = torch.cat((z, c), dim=1)
        rnn_state = self.proj_network(zc) # Todo: how to get state?
